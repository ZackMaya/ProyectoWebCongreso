(function($){
  ...
  $('ul#menuajax a').each(function(){ //Cambiamos los href por el contenido del atributo data-hash
    $(this).attr('href', $(this).data('hash'));
  });

  $('ul#menuajax a').on('click', function(e){          
  ...
  function revisarURL (hash){
    var deferred = $.Deferred(); //deferred jquery
    ...
    if (hash !== url_anterior){ 
      ...
      cargarPagina(hash).done(
        ...
        deferred.reject('<p>Pagina en construcción, gracias por tu comprensión</p>'); // Rechazamos nuestro deferred      
      });
    }
